# LIBRARY STUDY LINEAGE / VERSIONING ENGINE EXECUTION 07 — BUILD AFFECTED-VERSION LOOKUP + BUILD INCREMENTAL VERSION CREATION WORKFLOW + HANDLE NON-MATERIAL SOURCE UPDATES

## Execution Identity

- **Execution:** Library Study Lineage / Versioning Engine Execution 07
- **Roadmap tasks embedded:** 14, 15, 16
- **Roadmap phase:** PHASE III — LIVING REPOSITORY STUDY WORKFLOW
...
Deliver one coherent Library Study Lineage / Versioning Engine execution slice covering **Build affected-version lookup + Build incremental version creation workflow + Handle non-material source updates**. Each Child Loop must reach `YES`; then perform an integrated review and fresh final-state verification. The Engine must remain backend/terminal-first and game-independent.
...
# CHILD LOOP 1 — TASK 14: Build affected-version lookup
...
Implement **Task 14: Build affected-version lookup** as a coherent repository-native capability, adapting details to verified current architecture while preserving the embedded functional intent.

## Final Goal for This Child Loop

An independent reviewer can inspect the final repository and fresh evidence and conclude that **Build affected-version lookup** is implemented through canonical owners, verified under relevant failure/boundary conditions, and does not leave unjustified duplicate architecture.
...
# CHILD LOOP 2 — TASK 15: Build incremental version creation workflow
...
Implement **Task 15: Build incremental version creation workflow** as a coherent repository-native capability, adapting details to verified current architecture while preserving the embedded functional intent.

## Final Goal for This Child Loop

An independent reviewer can inspect the final repository and fresh evidence and conclude that **Build incremental version creation workflow** is implemented through canonical owners, verified under relevant failure/boundary conditions, and does not leave unjustified duplicate architecture.
...
# CHILD LOOP 3 — TASK 16: Handle non-material source updates
...
Implement **Task 16: Handle non-material source updates** as a coherent repository-native capability, adapting details to verified current architecture while preserving the embedded functional intent.

## Final Goal for This Child Loop

An independent reviewer can inspect the final repository and fresh evidence and conclude that **Handle non-material source updates** is implemented through canonical owners, verified under relevant failure/boundary conditions, and does not leave unjustified duplicate architecture.
