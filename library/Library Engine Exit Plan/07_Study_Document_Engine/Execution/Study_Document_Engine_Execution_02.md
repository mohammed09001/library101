# LIBRARY STUDY / DOCUMENT ENGINE EXECUTION 02 — DEFINE EVIDENCE-REFERENCE SCHEMA + DEFINE STUDY INTERMEDIATE REPRESENTATION + PUBLISH STUDY CONTRACTS/EVENTS

## Execution Identity

- **Execution:** Library Study / Document Engine Execution 02
- **Roadmap tasks embedded:** 4, 5, 6
- **Roadmap phase:** PHASE I — STUDY ARTIFACT AND SCHEMA FOUNDATION
...
Deliver one coherent Library Study / Document Engine execution slice covering **Define evidence-reference schema + Define Study intermediate representation + Publish Study contracts/events**. Each Child Loop must reach `YES`; then perform an integrated review and fresh final-state verification. The Engine must remain backend/terminal-first and game-independent.
...
- **Why this Execution cares:** Plugin-based parse → syntax tree → transform → compile pipeline across mdast/hast/nlcst ecosystems, with file metadata via vfile. Use a structured Study IR/AST and deterministic renderers.
- **Inspect at minimum:**
...
# CHILD LOOP 1 — TASK 4: Define evidence-reference schema
...
Implement **Task 4: Define evidence-reference schema** as a coherent repository-native capability, adapting details to verified current architecture while preserving the embedded functional intent.

## Final Goal for This Child Loop

An independent reviewer can inspect the final repository and fresh evidence and conclude that **Define evidence-reference schema** is implemented through canonical owners, verified under relevant failure/boundary conditions, and does not leave unjustified duplicate architecture.
...
# CHILD LOOP 2 — TASK 5: Define Study intermediate representation
...
Implement **Task 5: Define Study intermediate representation** as a coherent repository-native capability, adapting details to verified current architecture while preserving the embedded functional intent.

## Final Goal for This Child Loop

An independent reviewer can inspect the final repository and fresh evidence and conclude that **Define Study intermediate representation** is implemented through canonical owners, verified under relevant failure/boundary conditions, and does not leave unjustified duplicate architecture.
...
- **unifiedjs/unified** — inspect processor parse/run/stringify, plugin contracts, mdast/remark ecosystem, vfile metadata, bridge vs mutate transformations.
...
# CHILD LOOP 3 — TASK 6: Publish Study contracts/events
...
Implement **Task 6: Publish Study contracts/events** as a coherent repository-native capability, adapting details to verified current architecture while preserving the embedded functional intent.

## Final Goal for This Child Loop

An independent reviewer can inspect the final repository and fresh evidence and conclude that **Publish Study contracts/events** is implemented through canonical owners, verified under relevant failure/boundary conditions, and does not leave unjustified duplicate architecture.
