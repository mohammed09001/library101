# LIBRARY STUDY / DOCUMENT ENGINE EXECUTION 08 — BUILD USER ANNOTATION MODEL + BUILD ANNOTATION MERGE INTO RENDER VIEWS + BUILD PANDOC EXPORT ADAPTER

## Execution Identity

- **Execution:** Library Study / Document Engine Execution 08
- **Roadmap tasks embedded:** 19, 20, 21
- **Roadmap phase:** PHASE IV — NOTES, EXPORT, AND CROSS-ENGINE HANDOFFS
...
- **Execution order:** Task 19 → Task 20 → Task 21
...
Deliver one coherent Library Study / Document Engine execution slice covering **Build user annotation model + Build annotation merge into render views + Build Pandoc export adapter**. Each Child Loop must reach `YES`; then perform an integrated review and fresh final-state verification. The Engine must remain backend/terminal-first and game-independent.
...
- **Why this Execution cares:** Reader → AST → filter → writer model and broad document export. Keep Pandoc outside canonical Study ownership; export from a validated Library Study representation.
- **Inspect at minimum:**
  - Pandoc AST
  - JSON/Lua filters
  - reader/writer model
...
# CHILD LOOP 1 — TASK 19: Build user annotation model
...
Implement **Task 19: Build user annotation model** as a coherent repository-native capability, adapting details to verified current architecture while preserving the embedded functional intent.

## Final Goal for This Child Loop

An independent reviewer can inspect the final repository and fresh evidence and conclude that **Build user annotation model** is implemented through canonical owners, verified under relevant failure/boundary conditions, and does not leave unjustified duplicate architecture.
...
# CHILD LOOP 2 — TASK 20: Build annotation merge into render views
...
Implement **Task 20: Build annotation merge into render views** as a coherent repository-native capability, adapting details to verified current architecture while preserving the embedded functional intent.

## Final Goal for This Child Loop

An independent reviewer can inspect the final repository and fresh evidence and conclude that **Build annotation merge into render views** is implemented through canonical owners, verified under relevant failure/boundary conditions, and does not leave unjustified duplicate architecture.
...
# CHILD LOOP 3 — TASK 21: Build Pandoc export adapter
...
Implement **Task 21: Build Pandoc export adapter** as a coherent repository-native capability, adapting details to verified current architecture while preserving the embedded functional intent.

## Final Goal for This Child Loop

An independent reviewer can inspect the final repository and fresh evidence and conclude that **Build Pandoc export adapter** is implemented through canonical owners, verified under relevant failure/boundary conditions, and does not leave unjustified duplicate architecture.
...
- **jgm/pandoc** — inspect Pandoc AST, JSON/Lua filters, reader/writer model, metadata/citations, deterministic export considerations.
