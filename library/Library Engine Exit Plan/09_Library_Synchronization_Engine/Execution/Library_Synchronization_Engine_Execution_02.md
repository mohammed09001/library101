# LIBRARY SYNCHRONIZATION ENGINE EXECUTION 02 — DEFINE CLIENT IDENTITY AND CURSOR STATE + DEFINE SNAPSHOT/PROJECTION CONTRACT + PUBLISH SYNC API/EVENTS

## Execution Identity

- **Execution:** Library Synchronization Engine Execution 02
...
Deliver one coherent Library Synchronization Engine execution slice covering **Define client identity and cursor state + Define snapshot/projection contract + Publish Sync API/events**. Each Child Loop must reach `YES`; then perform an integrated review and fresh final-state verification. The Engine must remain backend/terminal-first and game-independent.
...
# CHILD LOOP 1 — TASK 4: Define client identity and cursor state
...
Implement **Task 4: Define client identity and cursor state** as a coherent repository-native capability, adapting details to verified current architecture while preserving the embedded functional intent.

## Final Goal for This Child Loop

An independent reviewer can inspect the final repository and fresh evidence and conclude that **Define client identity and cursor state** is implemented through canonical owners, verified under relevant failure/boundary conditions, and does not leave unjustified duplicate architecture.
...
# CHILD LOOP 2 — TASK 5: Define snapshot/projection contract
...
Implement **Task 5: Define snapshot/projection contract** as a coherent repository-native capability, adapting details to verified current architecture while preserving the embedded functional intent.

## Final Goal for This Child Loop

An independent reviewer can inspect the final repository and fresh evidence and conclude that **Define snapshot/projection contract** is implemented through canonical owners, verified under relevant failure/boundary conditions, and does not leave unjustified duplicate architecture.
...
# CHILD LOOP 3 — TASK 6: Publish Sync API/events
...
Implement **Task 6: Publish Sync API/events** as a coherent repository-native capability, adapting details to verified current architecture while preserving the embedded functional intent.

## Final Goal for This Child Loop

An independent reviewer can inspect the final repository and fresh evidence and conclude that **Publish Sync API/events** is implemented through canonical owners, verified under relevant failure/boundary conditions, and does not leave unjustified duplicate architecture.
