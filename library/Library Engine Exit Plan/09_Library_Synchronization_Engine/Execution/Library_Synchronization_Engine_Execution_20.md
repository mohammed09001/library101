# LIBRARY SYNCHRONIZATION ENGINE EXECUTION 20 — INSTRUMENT SYNC HEALTH + BUILD THE SYNC CLI

## Execution Identity

- **Execution:** Library Synchronization Engine Execution 20
...
Deliver one coherent Library Synchronization Engine execution slice covering **Instrument sync health + Build the Sync CLI**. Each Child Loop must reach `YES`; then perform an integrated review and fresh final-state verification. The Engine must remain backend/terminal-first and game-independent.
...
# CHILD LOOP 1 — TASK 36: Instrument sync health
...
Implement **Task 36: Instrument sync health** as a coherent repository-native capability, adapting details to verified current architecture while preserving the embedded functional intent.
...
# CHILD LOOP 2 — TASK 37: Build the Sync CLI
...
Implement **Task 37: Build the Sync CLI** as a coherent repository-native capability, adapting details to verified current architecture while preserving the embedded functional intent.

## Final Goal for This Child Loop

An independent reviewer can inspect the final repository and fresh evidence and conclude that **Build the Sync CLI** is implemented through canonical owners, verified under relevant failure/boundary conditions, and does not leave unjustified duplicate architecture.
