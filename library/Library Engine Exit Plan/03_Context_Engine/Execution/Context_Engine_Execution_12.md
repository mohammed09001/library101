# LIBRARY CONTEXT ENGINE EXECUTION 12 — BUILD THE CONTEXT CLI + EXPOSE MCP/HOST-NATIVE CONTEXT TOOLS

## Execution Identity

- **Execution:** Library Context Engine Execution 12
- **Roadmap tasks embedded:** 29, 30
- **Roadmap phase:** PHASE V — TERMINAL AND AGENT TOOLING
...
Deliver one coherent Library Context Engine execution slice covering **Build the Context CLI + Expose MCP/host-native Context tools**. Each Child Loop must reach `YES`; then perform an integrated review and fresh final-state verification. The Engine must remain backend/terminal-first and game-independent.
...
# CHILD LOOP 1 — TASK 29: Build the Context CLI
...
Implement **Task 29: Build the Context CLI** as a coherent repository-native capability, adapting details to verified current architecture while preserving the embedded functional intent.

## Final Goal for This Child Loop

An independent reviewer can inspect the final repository and fresh evidence and conclude that **Build the Context CLI** is implemented through canonical owners, verified under relevant failure/boundary conditions, and does not leave unjustified duplicate architecture.
...
# CHILD LOOP 2 — TASK 30: Expose MCP/host-native Context tools
...
Implement **Task 30: Expose MCP/host-native Context tools** as a coherent repository-native capability, adapting details to verified current architecture while preserving the embedded functional intent.

## Final Goal for This Child Loop

An independent reviewer can inspect the final repository and fresh evidence and conclude that **Expose MCP/host-native Context tools** is implemented through canonical owners, verified under relevant failure/boundary conditions, and does not leave unjustified duplicate architecture.
