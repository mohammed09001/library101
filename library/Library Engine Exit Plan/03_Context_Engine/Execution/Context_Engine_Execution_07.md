# LIBRARY CONTEXT ENGINE EXECUTION 07 — BUILD DIVERSITY AND COVERAGE POLICY + BUILD EXPLICIT TOKEN/BYTE BUDGET ACCOUNTING + BUILD PRIORITY AND PINNING SEMANTICS

## Execution Identity

- **Execution:** Library Context Engine Execution 07
- **Roadmap tasks embedded:** 18, 19, 20
- **Roadmap phase:** PHASE III — RANKING, BUDGETING, AND PACK ASSEMBLY
...
Deliver one coherent Library Context Engine execution slice covering **Build diversity and coverage policy + Build explicit token/byte budget accounting + Build priority and pinning semantics**. Each Child Loop must reach `YES`; then perform an integrated review and fresh final-state verification. The Engine must remain backend/terminal-first and game-independent.
...
- **Why this Execution cares:** RepoMap selects important symbols and signatures across a codebase, ranks dependency-connected files, and stays inside an explicit token budget. Context should select and compose rather than dump the repository.
- **Inspect at minimum:**
  - aider/repomap.py
  - repository-map docs
...
# CHILD LOOP 1 — TASK 18: Build diversity and coverage policy
...
Implement **Task 18: Build diversity and coverage policy** as a coherent repository-native capability, adapting details to verified current architecture while preserving the embedded functional intent.

## Final Goal for This Child Loop

An independent reviewer can inspect the final repository and fresh evidence and conclude that **Build diversity and coverage policy** is implemented through canonical owners, verified under relevant failure/boundary conditions, and does not leave unjustified duplicate architecture.
...
# CHILD LOOP 2 — TASK 19: Build explicit token/byte budget accounting
...
Implement **Task 19: Build explicit token/byte budget accounting** as a coherent repository-native capability, adapting details to verified current architecture while preserving the embedded functional intent.

## Final Goal for This Child Loop

An independent reviewer can inspect the final repository and fresh evidence and conclude that **Build explicit token/byte budget accounting** is implemented through canonical owners, verified under relevant failure/boundary conditions, and does not leave unjustified duplicate architecture.
...
- **Aider-AI/aider** — inspect aider/repomap.py, repository-map docs, tree-sitter/ctags usage, graph ranking/PageRank-style relevance, map token budgeting.
...
# CHILD LOOP 3 — TASK 20: Build priority and pinning semantics
...
Implement **Task 20: Build priority and pinning semantics** as a coherent repository-native capability, adapting details to verified current architecture while preserving the embedded functional intent.

## Final Goal for This Child Loop

An independent reviewer can inspect the final repository and fresh evidence and conclude that **Build priority and pinning semantics** is implemented through canonical owners, verified under relevant failure/boundary conditions, and does not leave unjustified duplicate architecture.
...
Answer exactly: **HAVE WE ACHIEVED THE FINAL GOAL FOR TASK 20? — YES / PARTIALLY / NO**
