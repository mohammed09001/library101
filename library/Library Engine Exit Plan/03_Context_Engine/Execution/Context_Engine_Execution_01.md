# LIBRARY CONTEXT ENGINE EXECUTION 01 — FREEZE THE CONTEXT ENGINE BOUNDARY + DEFINE CONTEXTREQUEST AND TASK INTENT SCHEMA + DEFINE THE CONTEXT PROVIDER CONTRACT

## Execution Identity

- **Execution:** Library Context Engine Execution 01
- **Roadmap tasks embedded:** 1, 2, 3
- **Roadmap phase:** PHASE I — CONTEXT PRODUCT AND CONTRACT FOUNDATION
...
Deliver one coherent Library Context Engine execution slice covering **Freeze the Context Engine boundary + Define ContextRequest and task intent schema + Define the Context Provider contract**. Each Child Loop must reach `YES`; then perform an integrated review and fresh final-state verification. The Engine must remain backend/terminal-first and game-independent.
...
- **Why this Execution cares:** Separate context providers for files, search, Git commits, databases, web, issues, current file, and file tree. Preserve provider capability isolation and fail-soft behavior while avoiding legacy/deprecated provider patterns.
- **Inspect at minimum:**
...
# CHILD LOOP 1 — TASK 1: Freeze the Context Engine boundary
...
Implement **Task 1: Freeze the Context Engine boundary** as a coherent repository-native capability, adapting details to verified current architecture while preserving the embedded functional intent.

## Final Goal for This Child Loop

An independent reviewer can inspect the final repository and fresh evidence and conclude that **Freeze the Context Engine boundary** is implemented through canonical owners, verified under relevant failure/boundary conditions, and does not leave unjustified duplicate architecture.
...
Answer exactly: **HAVE WE ACHIEVED THE FINAL GOAL FOR TASK 1? — YES / PARTIALLY / NO**
...
# CHILD LOOP 2 — TASK 2: Define ContextRequest and task intent schema
...
Implement **Task 2: Define ContextRequest and task intent schema** as a coherent repository-native capability, adapting details to verified current architecture while preserving the embedded functional intent.

## Final Goal for This Child Loop

An independent reviewer can inspect the final repository and fresh evidence and conclude that **Define ContextRequest and task intent schema** is implemented through canonical owners, verified under relevant failure/boundary conditions, and does not leave unjustified duplicate architecture.
...
Answer exactly: **HAVE WE ACHIEVED THE FINAL GOAL FOR TASK 2? — YES / PARTIALLY / NO**

Advance only on `YES` with fresh evidence. On an irreducible blocker, stop dependent work and preserve the safest truthful state.

---

# CHILD LOOP 3 — TASK 3: Define the Context Provider contract
...
Implement **Task 3: Define the Context Provider contract** as a coherent repository-native capability, adapting details to verified current architecture while preserving the embedded functional intent.

## Final Goal for This Child Loop

An independent reviewer can inspect the final repository and fresh evidence and conclude that **Define the Context Provider contract** is implemented through canonical owners, verified under relevant failure/boundary conditions, and does not leave unjustified duplicate architecture.
...
- **continuedev/continue** — inspect core/context/providers, provider interface/types, SearchContextProvider, GitCommitContextProvider, FileTreeContextProvider.
...
Answer exactly: **HAVE WE ACHIEVED THE FINAL GOAL FOR TASK 3? — YES / PARTIALLY / NO**
