# LIBRARY CONTEXT ENGINE EXECUTION 02 — DEFINE NORMALIZED CONTEXTCANDIDATE SCHEMA + DEFINE CONTEXTPACK SCHEMA AND IMMUTABLE BUILD RECORD + PUBLISH VERSIONED CONTEXT CONTRACTS AND EVENTS

## Execution Identity

- **Execution:** Library Context Engine Execution 02
...
Deliver one coherent Library Context Engine execution slice covering **Define normalized ContextCandidate schema + Define ContextPack schema and immutable build record + Publish versioned Context contracts and events**. Each Child Loop must reach `YES`; then perform an integrated review and fresh final-state verification. The Engine must remain backend/terminal-first and game-independent.
...
# CHILD LOOP 1 — TASK 4: Define normalized ContextCandidate schema
...
Implement **Task 4: Define normalized ContextCandidate schema** as a coherent repository-native capability, adapting details to verified current architecture while preserving the embedded functional intent.

## Final Goal for This Child Loop

An independent reviewer can inspect the final repository and fresh evidence and conclude that **Define normalized ContextCandidate schema** is implemented through canonical owners, verified under relevant failure/boundary conditions, and does not leave unjustified duplicate architecture.
...
# CHILD LOOP 2 — TASK 5: Define ContextPack schema and immutable build record
...
Implement **Task 5: Define ContextPack schema and immutable build record** as a coherent repository-native capability, adapting details to verified current architecture while preserving the embedded functional intent.

## Final Goal for This Child Loop

An independent reviewer can inspect the final repository and fresh evidence and conclude that **Define ContextPack schema and immutable build record** is implemented through canonical owners, verified under relevant failure/boundary conditions, and does not leave unjustified duplicate architecture.
...
# CHILD LOOP 3 — TASK 6: Publish versioned Context contracts and events
...
Implement **Task 6: Publish versioned Context contracts and events** as a coherent repository-native capability, adapting details to verified current architecture while preserving the embedded functional intent.

## Final Goal for This Child Loop

An independent reviewer can inspect the final repository and fresh evidence and conclude that **Publish versioned Context contracts and events** is implemented through canonical owners, verified under relevant failure/boundary conditions, and does not leave unjustified duplicate architecture.
