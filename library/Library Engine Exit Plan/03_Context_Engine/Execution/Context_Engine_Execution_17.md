# LIBRARY CONTEXT ENGINE EXECUTION 17 — INSTRUMENT CONTEXT QUALITY AND COST

## Execution Identity

- **Execution:** Library Context Engine Execution 17
...
Deliver one coherent Library Context Engine execution slice covering **Instrument Context quality and cost**. Each Child Loop must reach `YES`; then perform an integrated review and fresh final-state verification. The Engine must remain backend/terminal-first and game-independent.
...
# CHILD LOOP 1 — TASK 37: Instrument Context quality and cost
...
Implement **Task 37: Instrument Context quality and cost** as a coherent repository-native capability, adapting details to verified current architecture while preserving the embedded functional intent.

## Final Goal for This Child Loop

An independent reviewer can inspect the final repository and fresh evidence and conclude that **Instrument Context quality and cost** is implemented through canonical owners, verified under relevant failure/boundary conditions, and does not leave unjustified duplicate architecture.
