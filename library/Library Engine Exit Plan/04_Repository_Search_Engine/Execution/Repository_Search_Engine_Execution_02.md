# LIBRARY REPOSITORY SEARCH ENGINE EXECUTION 02 — DEFINE STABLE REPOSITORY IDENTITY AND REVISION + DEFINE SEARCHSESSION, CANDIDATE AND RANKING SCHEMAS + PUBLISH SEARCH API/EVENTS

## Execution Identity

- **Execution:** Library Repository Search Engine Execution 02
- **Roadmap tasks embedded:** 4, 5, 6
- **Roadmap phase:** PHASE I — SEARCH PRODUCT AND DOMAIN FOUNDATION
...
Deliver one coherent Library Repository Search Engine execution slice covering **Define stable repository identity and revision + Define SearchSession, Candidate and Ranking schemas + Publish Search API/events**. Each Child Loop must reach `YES`; then perform an integrated review and fresh final-state verification. The Engine must remain backend/terminal-first and game-independent.
...
# CHILD LOOP 1 — TASK 4: Define stable repository identity and revision
...
Implement **Task 4: Define stable repository identity and revision** as a coherent repository-native capability, adapting details to verified current architecture while preserving the embedded functional intent.

## Final Goal for This Child Loop

An independent reviewer can inspect the final repository and fresh evidence and conclude that **Define stable repository identity and revision** is implemented through canonical owners, verified under relevant failure/boundary conditions, and does not leave unjustified duplicate architecture.
...
# CHILD LOOP 2 — TASK 5: Define SearchSession, Candidate and Ranking schemas
...
Implement **Task 5: Define SearchSession, Candidate and Ranking schemas** as a coherent repository-native capability, adapting details to verified current architecture while preserving the embedded functional intent.

## Final Goal for This Child Loop

An independent reviewer can inspect the final repository and fresh evidence and conclude that **Define SearchSession, Candidate and Ranking schemas** is implemented through canonical owners, verified under relevant failure/boundary conditions, and does not leave unjustified duplicate architecture.
...
# CHILD LOOP 3 — TASK 6: Publish Search API/events
...
Implement **Task 6: Publish Search API/events** as a coherent repository-native capability, adapting details to verified current architecture while preserving the embedded functional intent.

## Final Goal for This Child Loop

An independent reviewer can inspect the final repository and fresh evidence and conclude that **Publish Search API/events** is implemented through canonical owners, verified under relevant failure/boundary conditions, and does not leave unjustified duplicate architecture.
