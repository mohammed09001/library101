# LIBRARY REPOSITORY SEARCH ENGINE EXECUTION 18 — BUILD UNTRUSTED REPOSITORY-CONTENT BOUNDARY + ENFORCE SOURCEBOT STUDY-ONLY LICENSING BOUNDARY

## Execution Identity

- **Execution:** Library Repository Search Engine Execution 18
- **Roadmap tasks embedded:** 39, 40
- **Roadmap phase:** PHASE VII — TERMINAL/AGENT INTERFACE AND RESEARCH SAFETY
...
- **Execution order:** Task 39 → Task 40
...
Deliver one coherent Library Repository Search Engine execution slice covering **Build untrusted repository-content boundary + Enforce Sourcebot study-only licensing boundary**. Each Child Loop must reach `YES`; then perform an integrated review and fresh final-state verification. The Engine must remain backend/terminal-first and game-independent.
...
# CHILD LOOP 1 — TASK 39: Build untrusted repository-content boundary
...
Implement **Task 39: Build untrusted repository-content boundary** as a coherent repository-native capability, adapting details to verified current architecture while preserving the embedded functional intent.

## Final Goal for This Child Loop

An independent reviewer can inspect the final repository and fresh evidence and conclude that **Build untrusted repository-content boundary** is implemented through canonical owners, verified under relevant failure/boundary conditions, and does not leave unjustified duplicate architecture.
...
# CHILD LOOP 2 — TASK 40: Enforce Sourcebot study-only licensing boundary
...
Implement **Task 40: Enforce Sourcebot study-only licensing boundary** as a coherent repository-native capability, adapting details to verified current architecture while preserving the embedded functional intent.

## Final Goal for This Child Loop

An independent reviewer can inspect the final repository and fresh evidence and conclude that **Enforce Sourcebot study-only licensing boundary** is implemented through canonical owners, verified under relevant failure/boundary conditions, and does not leave unjustified duplicate architecture.
...
- **sourcebot-dev/sourcebot** — inspect code-search ↔ reasoning boundary, citation/evidence UX, Zoekt protocol integration, repository configuration model, LICENSE.md before any reuse decision.
