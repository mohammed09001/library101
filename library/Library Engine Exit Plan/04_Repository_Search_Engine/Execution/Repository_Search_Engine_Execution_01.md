# LIBRARY REPOSITORY SEARCH ENGINE EXECUTION 01 — FREEZE SEARCH VS ANALYSIS OWNERSHIP + DEFINE SEARCHREQUEST AND SEARCHINTENT + DEFINE REPOSITORYPROVIDER CONTRACT

## Execution Identity

- **Execution:** Library Repository Search Engine Execution 01
...
Deliver one coherent Library Repository Search Engine execution slice covering **Freeze Search vs Analysis ownership + Define SearchRequest and SearchIntent + Define RepositoryProvider contract**. Each Child Loop must reach `YES`; then perform an integrated review and fresh final-state verification. The Engine must remain backend/terminal-first and game-independent.
...
# CHILD LOOP 1 — TASK 1: Freeze Search vs Analysis ownership
...
Implement **Task 1: Freeze Search vs Analysis ownership** as a coherent repository-native capability, adapting details to verified current architecture while preserving the embedded functional intent.

## Final Goal for This Child Loop

An independent reviewer can inspect the final repository and fresh evidence and conclude that **Freeze Search vs Analysis ownership** is implemented through canonical owners, verified under relevant failure/boundary conditions, and does not leave unjustified duplicate architecture.
...
# CHILD LOOP 2 — TASK 2: Define SearchRequest and SearchIntent
...
Implement **Task 2: Define SearchRequest and SearchIntent** as a coherent repository-native capability, adapting details to verified current architecture while preserving the embedded functional intent.

## Final Goal for This Child Loop

An independent reviewer can inspect the final repository and fresh evidence and conclude that **Define SearchRequest and SearchIntent** is implemented through canonical owners, verified under relevant failure/boundary conditions, and does not leave unjustified duplicate architecture.
...
# CHILD LOOP 3 — TASK 3: Define RepositoryProvider contract
...
Implement **Task 3: Define RepositoryProvider contract** as a coherent repository-native capability, adapting details to verified current architecture while preserving the embedded functional intent.

## Final Goal for This Child Loop

An independent reviewer can inspect the final repository and fresh evidence and conclude that **Define RepositoryProvider contract** is implemented through canonical owners, verified under relevant failure/boundary conditions, and does not leave unjustified duplicate architecture.
