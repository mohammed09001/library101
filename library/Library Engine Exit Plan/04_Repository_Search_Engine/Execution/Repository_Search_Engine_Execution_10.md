# LIBRARY REPOSITORY SEARCH ENGINE EXECUTION 10 — DEFINE NORMALIZED REPOSITORYPROFILE + BUILD TECHNICAL/ARCHITECTURE SIGNAL EXTRACTORS + BUILD INTENT-MATCH SCORING BASELINE

## Execution Identity

- **Execution:** Library Repository Search Engine Execution 10
- **Roadmap tasks embedded:** 25, 26, 27
- **Roadmap phase:** PHASE V — PROFILE, SIGNALS, AND EXPLAINABLE RANKING
...
Deliver one coherent Library Repository Search Engine execution slice covering **Define normalized RepositoryProfile + Build technical/architecture signal extractors + Build intent-match scoring baseline**. Each Child Loop must reach `YES`; then perform an integrated review and fresh final-state verification. The Engine must remain backend/terminal-first and game-independent.
...
# CHILD LOOP 1 — TASK 25: Define normalized RepositoryProfile
...
Implement **Task 25: Define normalized RepositoryProfile** as a coherent repository-native capability, adapting details to verified current architecture while preserving the embedded functional intent.

## Final Goal for This Child Loop

An independent reviewer can inspect the final repository and fresh evidence and conclude that **Define normalized RepositoryProfile** is implemented through canonical owners, verified under relevant failure/boundary conditions, and does not leave unjustified duplicate architecture.
...
# CHILD LOOP 2 — TASK 26: Build technical/architecture signal extractors
...
Implement **Task 26: Build technical/architecture signal extractors** as a coherent repository-native capability, adapting details to verified current architecture while preserving the embedded functional intent.

## Final Goal for This Child Loop

An independent reviewer can inspect the final repository and fresh evidence and conclude that **Build technical/architecture signal extractors** is implemented through canonical owners, verified under relevant failure/boundary conditions, and does not leave unjustified duplicate architecture.
...
# CHILD LOOP 3 — TASK 27: Build intent-match scoring baseline
...
Implement **Task 27: Build intent-match scoring baseline** as a coherent repository-native capability, adapting details to verified current architecture while preserving the embedded functional intent.

## Final Goal for This Child Loop

An independent reviewer can inspect the final repository and fresh evidence and conclude that **Build intent-match scoring baseline** is implemented through canonical owners, verified under relevant failure/boundary conditions, and does not leave unjustified duplicate architecture.
