# LIBRARY REPOSITORY SYNC ENGINE EXECUTION 11 — BUILD DETERMINISTIC CHANGE CLASSIFICATION + BUILD MATERIAL-CHANGE POLICY ENGINE + BUILD AFFECTED-STUDY LOOKUP CONTRACT

## Execution Identity

- **Execution:** Library Repository Sync Engine Execution 11
- **Roadmap tasks embedded:** 19, 20, 21
- **Roadmap phase:** PHASE IV — MATERIALITY AND INCREMENTAL TRIGGERS
...
Deliver one coherent Library Repository Sync Engine execution slice covering **Build deterministic change classification + Build material-change policy engine + Build affected-Study lookup contract**. Each Child Loop must reach `YES`; then perform an integrated review and fresh final-state verification. The Engine must remain backend/terminal-first and game-independent.
...
# CHILD LOOP 1 — TASK 19: Build deterministic change classification
...
Implement **Task 19: Build deterministic change classification** as a coherent repository-native capability, adapting details to verified current architecture while preserving the embedded functional intent.

## Final Goal for This Child Loop

An independent reviewer can inspect the final repository and fresh evidence and conclude that **Build deterministic change classification** is implemented through canonical owners, verified under relevant failure/boundary conditions, and does not leave unjustified duplicate architecture.
...
# CHILD LOOP 2 — TASK 20: Build material-change policy engine
...
Implement **Task 20: Build material-change policy engine** as a coherent repository-native capability, adapting details to verified current architecture while preserving the embedded functional intent.

## Final Goal for This Child Loop

An independent reviewer can inspect the final repository and fresh evidence and conclude that **Build material-change policy engine** is implemented through canonical owners, verified under relevant failure/boundary conditions, and does not leave unjustified duplicate architecture.
...
# CHILD LOOP 3 — TASK 21: Build affected-Study lookup contract
...
Implement **Task 21: Build affected-Study lookup contract** as a coherent repository-native capability, adapting details to verified current architecture while preserving the embedded functional intent.

## Final Goal for This Child Loop

An independent reviewer can inspect the final repository and fresh evidence and conclude that **Build affected-Study lookup contract** is implemented through canonical owners, verified under relevant failure/boundary conditions, and does not leave unjustified duplicate architecture.
