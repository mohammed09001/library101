# LIBRARY REPOSITORY SYNC ENGINE EXECUTION 02 — DEFINE SYNC POLICY SCHEMA + DEFINE SYNCSTATE STATE MACHINE + PUBLISH REPOSITORY SYNC CONTRACTS/EVENTS

## Execution Identity

- **Execution:** Library Repository Sync Engine Execution 02
...
Deliver one coherent Library Repository Sync Engine execution slice covering **Define sync policy schema + Define SyncState state machine + Publish Repository Sync contracts/events**. Each Child Loop must reach `YES`; then perform an integrated review and fresh final-state verification. The Engine must remain backend/terminal-first and game-independent.
...
# CHILD LOOP 1 — TASK 4: Define sync policy schema
...
Implement **Task 4: Define sync policy schema** as a coherent repository-native capability, adapting details to verified current architecture while preserving the embedded functional intent.

## Final Goal for This Child Loop

An independent reviewer can inspect the final repository and fresh evidence and conclude that **Define sync policy schema** is implemented through canonical owners, verified under relevant failure/boundary conditions, and does not leave unjustified duplicate architecture.
...
# CHILD LOOP 2 — TASK 5: Define SyncState state machine
...
Implement **Task 5: Define SyncState state machine** as a coherent repository-native capability, adapting details to verified current architecture while preserving the embedded functional intent.

## Final Goal for This Child Loop

An independent reviewer can inspect the final repository and fresh evidence and conclude that **Define SyncState state machine** is implemented through canonical owners, verified under relevant failure/boundary conditions, and does not leave unjustified duplicate architecture.
...
# CHILD LOOP 3 — TASK 6: Publish Repository Sync contracts/events
...
Implement **Task 6: Publish Repository Sync contracts/events** as a coherent repository-native capability, adapting details to verified current architecture while preserving the embedded functional intent.

## Final Goal for This Child Loop

An independent reviewer can inspect the final repository and fresh evidence and conclude that **Publish Repository Sync contracts/events** is implemented through canonical owners, verified under relevant failure/boundary conditions, and does not leave unjustified duplicate architecture.
