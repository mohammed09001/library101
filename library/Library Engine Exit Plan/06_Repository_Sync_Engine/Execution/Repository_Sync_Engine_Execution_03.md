# LIBRARY REPOSITORY SYNC ENGINE EXECUTION 03 — BUILD GITHUB REMOTE METADATA CHECKS + BUILD GITHUB COMPARE-COMMITS DELTA PATH + BUILD POLLING SCHEDULER WITH JITTER/BACKOFF

## Execution Identity

- **Execution:** Library Repository Sync Engine Execution 03
- **Roadmap tasks embedded:** 7, 8, 9
- **Roadmap phase:** PHASE II — REMOTE GITHUB CHANGE DETECTION
...
Deliver one coherent Library Repository Sync Engine execution slice covering **Build GitHub remote metadata checks + Build GitHub compare-commits delta path + Build polling scheduler with jitter/backoff**. Each Child Loop must reach `YES`; then perform an integrated review and fresh final-state verification. The Engine must remain backend/terminal-first and game-independent.
...
# CHILD LOOP 1 — TASK 7: Build GitHub remote metadata checks
...
Implement **Task 7: Build GitHub remote metadata checks** as a coherent repository-native capability, adapting details to verified current architecture while preserving the embedded functional intent.

## Final Goal for This Child Loop

An independent reviewer can inspect the final repository and fresh evidence and conclude that **Build GitHub remote metadata checks** is implemented through canonical owners, verified under relevant failure/boundary conditions, and does not leave unjustified duplicate architecture.
...
# CHILD LOOP 2 — TASK 8: Build GitHub compare-commits delta path
...
Implement **Task 8: Build GitHub compare-commits delta path** as a coherent repository-native capability, adapting details to verified current architecture while preserving the embedded functional intent.

## Final Goal for This Child Loop

An independent reviewer can inspect the final repository and fresh evidence and conclude that **Build GitHub compare-commits delta path** is implemented through canonical owners, verified under relevant failure/boundary conditions, and does not leave unjustified duplicate architecture.
...
# CHILD LOOP 3 — TASK 9: Build polling scheduler with jitter/backoff
...
Implement **Task 9: Build polling scheduler with jitter/backoff** as a coherent repository-native capability, adapting details to verified current architecture while preserving the embedded functional intent.

## Final Goal for This Child Loop

An independent reviewer can inspect the final repository and fresh evidence and conclude that **Build polling scheduler with jitter/backoff** is implemented through canonical owners, verified under relevant failure/boundary conditions, and does not leave unjustified duplicate architecture.
